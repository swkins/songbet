...
import {
  SUPPORTED_LEAGUES, fetchRecentCompletedMatches, fetchLiveMatch, fetchLiveCurrentGameId,
  fetchMatchGames, fetchGameDetailStats, fetchLiveGameStats,
...
function RawToggle({ raw }: { raw: unknown }) {
  ...
  return (
    <>
      <button type="button" onClick={() => setShow(p => !p)} style={{ fontSize: 9, color: 'var(--text-muted)', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>
        {show ? '원본 숨기기' : '원본 보기'}
      </button>
      {show && (
...
function DetailCard({ i, d }: { i: number; d: GameDetailStats | null }) {
  return (
    <div style={{ background: 'var(--bg-elevated)', borderRadius: 6, padding: '8px 10px', fontSize: 11 }}>
      ...
      {!d ? (
        <div style={{ color: 'var(--text-muted)' }}>상세 통계 없음 (이 경기는 상세 통계 커버리지가 없음)</div>
      ) : (
...
function MatchRow({ m }: { m: CompletedMatch }) {
  ...
  async function toggle() {
    ...
    if (next && details === null && !noGames) {
      setLoading(true)
      try {
        const games = (await fetchMatchGames(m.id)).sort((a, b) => a.number - b.number)
  ...
  return (
    <div style={{ borderBottom: '1px solid var(--border)' }}>
      <div onClick={toggle} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 4px', cursor: 'pointer' }}>
        ...
        <span style={{ fontSize: 10, color: 'var(--text-muted)', width: 44, flexShrink: 0 }}>{m.startTime ? dayjs(m.startTime).format('MM/DD') : '-'}</span>
      ...
      {open && (
        <div style={{ padding: '0 4px 12px 26px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ fontSize: 9 }}><RawToggle raw={m.raw} /></div>
          {noGames && (
...
function LiveTestPanel({ league }: { league: string }) {
  ...
  async function testDetail() {
    ...
    setLoading(true)
    try {
      const gameId = await fetchLiveCurrentGameId(live.id)
      if (!gameId) { setDetail('none'); setLoading(false); return }
      const d = await fetchLiveGameStats(gameId)
      setDetail(d)
    } catch {
      setDetail('none')
    }
  ...
  return (
    <div className="card" style={{ marginBottom: 12 }}>
      <div className="card-title" style={{ marginBottom: 6 }}>
        라이브 데이터 테스트 <span style={{ fontWeight: 400, color: 'var(--text-muted)', fontSize: 10 }}>· CitoAPI 연결 확인용</span>
      </div>
      ...
      {live && (
        <div>
          <div style={{ fontSize: 12, marginBottom: 6 }}>🔴 라이브: {live.teamACode || live.teamA} vs {live.teamBCode || live.teamB} <RawToggle raw={live.raw} /></div>
          ...
          {detail && detail !== 'none' && (
            <div style={{ fontSize: 11 }}>
              {detail.teamA.kills === 0 && detail.teamB.kills === 0 && detail.teamA.totalGold === 0 ? (
                <div style={{ color: 'var(--red)' }}>라이브인데도 전부 0으로 나옵니다 — 필드 매핑을 다시 확인해야 합니다.</div>
              ) : (
...
export default function Analysis() {
  ...
  function load() {
    ...
    fetchRecentCompletedMatches(league, 7)
      .then(setMatches)
      .catch(e => { setError(true); setErrorDetail(e instanceof Error ? e.message : String(e)) })
    ...
    <div className="page">
      ...
      <div className="card">
        <div className="card-title" style={{ marginBottom: 4 }}>
          최근 완료 경기 <span style={{ fontWeight: 400, color: 'var(--text-muted)', fontSize: 10 }}>· 최근 7일 · CitoAPI 기준 · 항목을 누르면 세트별 상세 통계</span>
        </div>
        {loading && <div style={{ fontSize: 12, color: 'var(--text-muted)', padding: '16px 0', textAlign: 'center' }}>불러오는 중...</div>}
        {error && <div style={{ fontSize: 12, color: 'var(--red)', padding: '16px 0', textAlign: 'center' }}>일정을 불러오지 못했습니다{errorDetail ? ` (${errorDetail})` : ''}</div>}
