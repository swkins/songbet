      ...
      {!loading && matches.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {visibleMatches.map(m => {
            ...
            // "입력됨 N" 배지: 날짜(±1일)로 먼저 후보를 추린 뒤, 상대팀(team2_name)이 이번 경기 상대와
            // 실제로 일치하는 것만 센다. 날짜만으로 판단하면 같은 팀이 인접일에 다른 상대와 치른 경기의
            // 기록까지 섞여서 "입력됨"으로 잘못 표시되는 문제가 있었다. team2_name은 저장할 때 항상
            // "풀네임"으로 남기므로(코드 아님), 비교도 코드가 아니라 스케줄의 풀네임(m.teamA/m.teamB)으로
            // 해야 정확히 맞는다 — 코드로 비교하면 "BFX"처럼 팀명(BNK FEARX Youth) 안에 코드 철자가
            // 그대로 안 들어있는 경우(LCK CL에서 흔함) 정상 기록도 "미입력"으로 잘못 표시된다.
            const oppQuery = resolved.isA ? m.teamB : m.teamA
            const recordCount = (recordedByTeam[resolved.teamId] ?? []).filter(s => {
              ...
              return inRange && isSameOpponent(s.team2_name, oppQuery, teams)
            }).length
            return (
...
function RecentMatchRow({ teamId, teamName, game, displayA, displayB, scoreA, scoreB, teams, recordCount, teamCode, opponentCode }: {
  ...
  async function loadSets() {
    ...
    // 일치하지 않으므로, 날짜 ±1일 범위로 먼저 넓게 찾는다. 수동 입력 데이터(정확한 match_start_time)도 이 범위 안에
    // 포함되므로 함께 잡힌다.
    // 다만 팀이 같은 날(또는 인접일)에 두 경기 이상 치르는 경우(더블헤더 등) team_id + 날짜만으로는 다른 상대와의
    // 경기 기록까지 같이 잡혀버리는 문제가 있었다 — 그래서 날짜로 후보를 추린 뒤, 상대팀(team2_name)이 이번 경기의
    // 상대(game.opponent/opponentCode)와 실제로 일치하는 것만 최종적으로 남긴다. 이름 표기 차이는
    // teamNameMatches(코드 별칭·정규화 포함)로 흡수하므로, 정상 저장된 기록이 새로고침 후 안 보이는 문제는 생기지 않는다.
    ...
    const { data } = await supabase.from('esports_game_stats').select('*')
      ...
      .order('game_number')
    const candidates = (data as EsportsGameStat[]) ?? []
    const filtered = candidates.filter(s => isSameOpponent(s.team2_name, game.opponent, teams) || (!!opponentCode && isSameOpponent(s.team2_name, opponentCode, teams)))
    setSets(filtered)
                  ...
                  {expanded && (
                    <div style={{ padding: '8px 8px', background: 'var(--bg-card)', borderRadius: 6, marginTop: 2, fontSize: 9 }}>
                      ...
                      <div style={{ color: 'var(--text-muted)', marginBottom: 6 }}>
                        최근 6개월 안의 경기만 반영합니다(그 전 경기는 DB엔 남지만 계산에서 제외). 최종 점수 = 반감기 60일 가중평균(최근폼, 80%) + 이 6개월 구간 단순평균(기본 체급, 20%) — 표본이 적을 때 노이즈를 눌러주는 안정판입니다. 연도가 바뀐 경기(예: 작년 데이터)는 로스터 개편을 감안해 가중치를 한 번 더 줄입니다. 경기 하나의 점수는 승패 + 그 경기 플레이 점수차(얼마나 압도적으로 이기고 졌는지)로 계산해서, 최종 {ps?.powerScore.toFixed(1)}점이 나왔습니다. 항목을 누르면 자세한 사유가 나옵니다.
                      </div>
