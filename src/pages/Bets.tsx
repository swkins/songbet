...
export default function Bets() {
  ...
  async function saveResult() {
    ...
    if (data) {
      await logAction({ action_type: 'update', table_name: 'bets', record_id: data.id, before_data: before as never, after_data: data as never, description: `결과: ${resultTarget.match} → ${resultValue === 'win' ? '적중' : resultValue === 'loss' ? '실패' : '적특'} (${profit >= 0 ? '+' : ''}${profit.toLocaleString()}원)` })
          ...
          <div key={site.id} className="site-section">
            ...
            {sb.length === 0 ? (
              <div className="site-section-table">
                <div style={{ padding: '16px', textAlign: 'center', color: 'var(--text-secondary)', fontSize: 12 }}>베팅 없음</div>
              </div>
            ) : (
              <div className="site-section-table">
                <table>
        ...
        <div className="modal-overlay" onClick={() => setShowSlip(false)}>
          <div className="betslip-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="betslip-modal-header">
              <div className="betslip-modal-title">BET SLIP</div>
              <div className="betslip-sites">
                {sites.map(s => (
                  <button key={s.id}
                    ...
                    {s.name}
                    <span className="site-bal">{s.balance.toLocaleString()}원</span>
                  </button>
                ))}
              ...
              <button className="btn btn-icon btn-ghost btn-sm" style={{ margin: '0 8px', flexShrink: 0 }}
            ...
            {activeSite ? (
              <div className="betslip-body">
                ...
                <div className="betslip-info-bar">
                  <div>
                    <div style={{ fontSize: 10, color: 'var(--text-secondary)', fontWeight: 700, letterSpacing: '0.5px', textTransform: 'uppercase' }}>잔액</div>
                    <div className={`fw-7 ${activeSite.balance >= 0 ? 'profit-pos' : 'profit-neg'}`}
                      style={{ fontFamily: 'var(--font-mono)', fontSize: 16, marginTop: 2 }}>
                      {activeSite.balance.toLocaleString()}원
                    </div>
                  </div>
                  {activeSite.rolling_target > 0 && (
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 10, color: 'var(--text-secondary)', fontWeight: 700, letterSpacing: '0.5px', textTransform: 'uppercase' }}>롤링</div>
                      <div style={{ fontSize: 13, color: 'var(--gold)', fontFamily: 'var(--font-mono)', fontWeight: 600, marginTop: 2 }}>
                        {rollingPct(activeSite)}%
                      </div>
                      <div className="rolling-bar" style={{ width: 80 }}>
                ...
                {/* 베팅 폼 */}
                <div className="betslip-form-grid">
                ...
                {/* 예상 수익 */}
                {slipForm.odds && slipForm.stake && Number(slipForm.odds) > 0 && Number(slipForm.stake) > 0 && (
                  <div style={{ padding: '8px 12px', background: 'var(--green-bg)', border: '1px solid var(--green-border)', borderRadius: 'var(--radius-sm)', fontSize: 12 }}>
                    예상 수익: <strong className="profit-pos">+{Math.round(Number(slipForm.stake) * (Number(slipForm.odds) - 1)).toLocaleString()}원</strong>
                    <span style={{ color: 'var(--text-secondary)', marginLeft: 8 }}>/ 합산 {(Number(slipForm.stake) * Number(slipForm.odds)).toLocaleString()}원 반환</span>
                  </div>
                )}

                {/* 입금 + 베팅 버튼 */}
                <div style={{ borderTop: '1px solid var(--border)', paddingTop: 12 }}>
                  <div style={{ fontSize: 10, color: 'var(--text-secondary)', marginBottom: 8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                    입금 / 베팅
                  </div>
                  <div className="betslip-action-row">
                    <input className="form-input" type="number" placeholder="금액 입력"
                      style={{ maxWidth: 160 }}
                      value={slipAmount} onChange={e => setSlipAmount(e.target.value)} />
                    <button className="btn btn-green" onClick={doDeposit}>입금</button>
                    <button className="btn btn-primary" onClick={doBet}>
                      <Check size={13} /> 베팅 등록
                    </button>
                  </div>
                  <div style={{ fontSize: 10, color: 'var(--text-secondary)', marginTop: 6 }}>
                    입금: 금액만큼 잔액 충전 · 베팅: 위 슬립 내용으로 베팅 등록 후 잔액 차감
                  </div>
                </div>
              </div>
            ) : (
