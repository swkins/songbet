...
// ─── 예측 신뢰도 (최근 데이터가 얼마나 쌓여있는지) ──────────────────────
// 최근 windowDays(기본 60일=2개월) 안에 이 팀이 치른 "시리즈"(세트 아님) 개수.
// EloGameLog는 세트마다 한 줄씩 남지만, gameNumber===1 항목만 세면 시리즈 1개당 정확히 1번 잡힌다.
export function computeRecentSeriesCount(log: EloGameLog[], teamId: string, asOfISO: string, windowDays = 60): number {
  const asOfMs = new Date(asOfISO).getTime()
  const cutoffMs = asOfMs - windowDays * 24 * 60 * 60 * 1000
  const set = new Set<string>()
  for (const l of log) {
    if (l.teamId !== teamId || l.gameNumber !== 1) continue
    const t = new Date(l.matchStartTime).getTime()
    if (t <= asOfMs && t >= cutoffMs) set.add(l.matchStartTime)
  }
  return set.size
}

// 신뢰도(0~1): 최근 2개월 안에 치른 시리즈가 maxSeries(기본 10)개 이상이면 100%로 최고치.
// 시즌이 끝나고 한두 달 쉬었다가 재개하면 최근 시리즈 수가 0에 가까우므로 신뢰도가 낮게 시작하고,
// 새 시즌 경기가 쌓일수록(최대 10경기까지) 다시 100%로 올라간다.
export function confidenceFromSeriesCount(count: number, maxSeries = 10): number {
  return Math.max(0, Math.min(1, count / maxSeries))
}

// ─── 파워랭킹 기반 4가지 베팅 유형 백테스트 ────────────────────────────
// (강팀 정배 / 강팀 -1.5 / 약팀 역배 / 약팀 +1.5) — 모두 BO3 기준.
export interface CompletedSeriesRecord {
  matchStartTime: string
  favTeamId: string
  dogTeamId: string
  favPowerBefore: number
  dogPowerBefore: number
  favSets: number
  dogSets: number
  favWonSeries: boolean
  favSwept: boolean    // 강팀 2:0 승 → "강팀 -1.5" 적중
  dogCovered: boolean  // 약팀 1세트 이상 획득(스윕 안 당함) → "약팀 +1.5" 적중
  outcome: SeriesOutcomeSummary
}

// EloMatchRecord(세트 단위, 한 방향으로 중복 제거된 목록)를 매치(시리즈) 단위로 묶어서
// BO3로 완결된(2승으로 종료) 시리즈만 골라낸다. 강팀/약팀은 그 시리즈가 "시작하기 전" 레이팅
// (EloGameLog의 gameNumber===1 ratingBefore) 기준으로 정한다 — 결과를 알고 사후적으로
// 유리하게 나누는 걸 방지하기 위함(백테스트의 핵심 전제).
export function buildCompletedSeries(matches: EloMatchRecord[], log: EloGameLog[]): CompletedSeriesRecord[] {
  const groups = new Map<string, EloMatchRecord[]>()
  for (const m of matches) {
    const key = `${m.teamAId}|${m.teamBId}|${m.matchStartTime}`
    if (!groups.has(key)) groups.set(key, [])
    groups.get(key)!.push(m)
  }
  const out: CompletedSeriesRecord[] = []
  for (const sets of groups.values()) {
    const teamAId = sets[0].teamAId, teamBId = sets[0].teamBId, matchStartTime = sets[0].matchStartTime
    const aWins = sets.filter(s => s.winnerIsA).length
    const bWins = sets.length - aWins
    // BO3로 끝난 경기만(2:0 또는 2:1). 3승 이상(BO5)이거나 1세트뿐인(BO1/입력 중) 경우는
    // -1.5/+1.5 핸디캡 의미 자체가 달라지므로 백테스트에서 제외.
    if (Math.max(aWins, bWins) !== 2) continue
    const ratingA = log.find(l => l.teamId === teamAId && l.matchStartTime === matchStartTime && l.gameNumber === 1)?.ratingBefore
    const ratingB = log.find(l => l.teamId === teamBId && l.matchStartTime === matchStartTime && l.gameNumber === 1)?.ratingBefore
    if (ratingA == null || ratingB == null) continue
    const favIsA = ratingA >= ratingB
    const favTeamId = favIsA ? teamAId : teamBId
    const dogTeamId = favIsA ? teamBId : teamAId
    const favPowerBefore = favIsA ? ratingA : ratingB
    const dogPowerBefore = favIsA ? ratingB : ratingA
    const favSets = favIsA ? aWins : bWins
    const dogSets = favIsA ? bWins : aWins
    const p = powerScoreMatchupProbability(favPowerBefore, dogPowerBefore)
    out.push({
      matchStartTime, favTeamId, dogTeamId, favPowerBefore, dogPowerBefore,
      favSets, dogSets, favWonSeries: favSets > dogSets, favSwept: favSets === 2 && dogSets === 0,
      dogCovered: dogSets >= 1, outcome: seriesOutcomeSummary(p, 3),
    })
  }
  return out.sort((a, b) => new Date(a.matchStartTime).getTime() - new Date(b.matchStartTime).getTime())
}

export interface BacktestCategory { hits: number; total: number }
export interface StrategyBacktestSummary {
  favML: BacktestCategory        // 강팀 정배
  favHandicap: BacktestCategory  // 강팀 -1.5
  dogML: BacktestCategory        // 약팀 역배
  dogHandicap: BacktestCategory  // 약팀 +1.5
  recommended: BacktestCategory  // 매 경기 "확률이 더 높은 쪽"에 모의베팅했을 때의 전체 적중률
}

// 4가지 베팅 유형 각각의 적중률과, "그때그때 모델 확률이 더 높은 쪽에 베팅"하는 전략의 적중률을 계산.
export function summarizeStrategyBacktest(series: CompletedSeriesRecord[]): StrategyBacktestSummary {
  const favML: BacktestCategory = { hits: 0, total: 0 }
  const favHandicap: BacktestCategory = { hits: 0, total: 0 }
  const dogML: BacktestCategory = { hits: 0, total: 0 }
  const dogHandicap: BacktestCategory = { hits: 0, total: 0 }
  const recommended: BacktestCategory = { hits: 0, total: 0 }
  for (const s of series) {
    favML.total++; if (s.favWonSeries) favML.hits++
    favHandicap.total++; if (s.favSwept) favHandicap.hits++
    dogML.total++; if (!s.favWonSeries) dogML.hits++
    dogHandicap.total++; if (s.dogCovered) dogHandicap.hits++

    // 그룹1: 강팀 정배 vs 약팀 +1.5 — 모델 확률이 더 높은 쪽에 베팅했다고 가정
    recommended.total++
    if (s.outcome.favWinProb >= s.outcome.underAtLeastOneGameProb) { if (s.favWonSeries) recommended.hits++ }
    else { if (s.dogCovered) recommended.hits++ }
    // 그룹2: 강팀 -1.5 vs 약팀 역배 — 마찬가지
    recommended.total++
    if (s.outcome.favSweepProb >= s.outcome.underWinProb) { if (s.favSwept) recommended.hits++ }
    else { if (!s.favWonSeries) recommended.hits++ }
  }
  return { favML, favHandicap, dogML, dogHandicap, recommended }
}
