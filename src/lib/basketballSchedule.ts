...
async function fetchLeagueRange(slug: string, fromCompact: string, toCompact: string): Promise<any[]> {
  try {
    ...
    if (!res.ok) { console.warn(`[basketballSchedule] ${slug} ${fromCompact}-${toCompact}: HTTP ${res.status}`); return [] }
    const json = await res.json()
    return json?.events ?? []
  } catch (err) {
    console.warn(`[basketballSchedule] ${slug} ${fromCompact}-${toCompact}: 호출 실패`, err)
    return []
  }
}
